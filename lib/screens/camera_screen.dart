...
class _CameraAnalysisScreenState extends State<CameraAnalysisScreen>
  ...
  Future<void> _startAnalysis() async {
    setState(() { _analyzing = true; _done = false; _result = null; _scanProgress = 0; });
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _analyzing = false; _done = true; _result = result; });
    } catch (e) {
      if (!mounted) return;
            ...
            child: ClipRRect(
              ...
              child: Stack(children: [
                ...
                CustomPaint(
                  ...
                  painter: _FaceGridPainter(progress: _scanProgress, done: _done),
                ),
                  ...
                  child: AnimatedBuilder(
                    ...
                    builder: (_, __) => Container(
                      ...
                      decoration: BoxDecoration(
                        border: Border.all(
                          color: _done
                              ? (_result?['stressLevel'] == 'HIGH' ? AppColors.red
                              : _result?['stressLevel'] == 'MEDIUM' ? AppColors.amber
                              : AppColors.green)
                              : AppColors.cyan.withOpacity(
                              _analyzing ? 0.4 + _pulseCtrl.value * 0.6 : 0.3),
                          width: 2,
                        ),
                    ...
                    child: Row(children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          color: _done ? AppColors.green
                              : _cameraReady ? (_analyzing ? AppColors.cyan : AppColors.amber)
                      ...
                      Expanded(
                        child: Text(
                          _done
                              ? (widget.showArabic ? 'اكتمل التحليل' : 'ANALYSIS COMPLETE')
                              : _analyzing
                              ? (widget.showArabic ? 'جارٍ التحليل...' : 'ANALYZING...')
                              : _cameraReady
                              ? (widget.showArabic ? 'الكاميرا جاهزة' : 'CAMERA READY')
                              : (widget.showArabic ? 'تهيئة الكاميرا...' : 'INITIALIZING...'),
                          style: GoogleFonts.shareTechMono(
                            fontSize: 10,
                            color: _done ? AppColors.green
                                : _cameraReady ? (_analyzing ? AppColors.cyan : AppColors.amber)
          ...
          if (_analyzing)
            ClipRRect(
              ...
              child: LinearProgressIndicator(
                value: _scanProgress, minHeight: 6,
          ...
          if (!_done)
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: (_analyzing || !_cameraReady) ? null : _startAnalysis,
                icon: _analyzing
                ...
                label: Text(
                  _analyzing
                      ...
                      : !_cameraReady
                      ? (widget.showArabic ? 'جارٍ تهيئة الكاميرا...' : 'INITIALIZING...')
                      : (widget.showArabic ? 'ابدأ مسح الوجه' : 'START FACE SCAN'),
                  style: GoogleFonts.shareTechMono(fontSize: 13, letterSpacing: 2),
                  overflow: TextOverflow.ellipsis,
                ),
                ...
                child: SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    label: Text(widget.showArabic ? 'إعادة المسح' : 'RESCAN',
                        style: GoogleFonts.shareTechMono(fontSize: 11, letterSpacing: 1.5)),
...
class _FaceGridPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final p = Paint()..color = AppColors.cyan.withOpacity(0.05)..strokeWidth = 0.5;
    const step = 30.0;
  ...
  @override bool shouldRepaint(_FaceGridPainter old) => old.progress != progress;
}
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(
                child: Text(
                  showArabic ? 'نتائج التحليل الحقيقي' : 'LIVE ANALYSIS RESULTS',
                  style: GoogleFonts.shareTechMono(
              ...
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                decoration: BoxDecoration(
                  color: (isReal ? AppColors.green : AppColors.amber).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(3),
                  border: Border.all(
                      color: (isReal ? AppColors.green : AppColors.amber).withOpacity(0.3)),
                ),
                child: Text(
            ...
            Row(children: [
              Expanded(child: _MetricTile(
                ...
                value: stressLevel, score: stressIdx,
                color: stressColor, icon: '🧠',
              )),
              const SizedBox(width: 8),
              Expanded(child: _MetricTile(
                label: showArabic ? 'الإرهاق' : 'FATIGUE',
                value: fatigueLevel, score: fatigueIdx,
                color: fatigueColor, icon: '😴',
              )),
              const SizedBox(width: 8),
              Expanded(child: _MetricTile(
                label: showArabic ? 'انتباه العين' : 'EYE',
                value: eyeScore > 70 ? 'ALERT' : eyeScore > 40 ? 'TIRED' : 'LOW',
                score: eyeScore,
                color: eyeScore > 70 ? AppColors.green
                    : eyeScore > 40 ? AppColors.amber : AppColors.red,
                icon: '👁️',
              )),
            ]),
            const SizedBox(height: 12),
            AProgressBar(
              value: stressIdx / 100.0, color: stressColor,
              label: showArabic ? 'الإجهاد: $stressIdx%' : 'Stress: $stressIdx%',
            ),
            const SizedBox(height: 6),
            AProgressBar(
              value: fatigueIdx / 100.0, color: fatigueColor,
              label: showArabic ? 'الإرهاق: $fatigueIdx%' : 'Fatigue: $fatigueIdx%',
            ),
            const SizedBox(height: 6),
            AProgressBar(
              ...
              color: eyeScore > 70 ? AppColors.green
                  : eyeScore > 40 ? AppColors.amber : AppColors.red,
              label: showArabic ? 'انتباه العين: $eyeScore%' : 'Eye Alertness: $eyeScore%',
            ),
            const SizedBox(height: 12),
            Container(
...
class _MetricTile extends StatelessWidget {
  ...
  const _MetricTile({required this.label, required this.value,
    required this.score, required this.color, required this.icon});
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      Text(label, style: const TextStyle(
          fontSize: 8, color: AppColors.textTertiary, letterSpacing: 0.5)),
    ]),
  );
}